  ...
  <div>
    <main class="mx-auto w-full max-w-3xl px-4 pt-6 pb-20 md:px-12 md:pt-10">
      <div class="flex items-end justify-between gap-4">
        <div>
          <h1 class="text-3xl md:text-[40px] font-bold leading-tight tracking-tight text-nt">音乐</h1>
          <p class="mt-1 text-sm text-nt-soft">单曲、专辑、随手听到喜欢的，都丢进来。</p>
        </div>
        <button
          type="button"
      ...
      <ul v-else class="mt-6 divide-y divide-nt-divider rounded-md border border-nt-divider bg-white">
        ...
        >
          ...
          <div class="min-w-0 flex-1">
            ...
            <div v-if="it.artist" class="truncate text-xs text-nt-soft">{{ it.artist }}</div>
    ...
    <div v-if="formOpen" class="fixed inset-0 z-40 flex items-center justify-center bg-black/30 px-4" @click.self="closeForm">
      <div class="w-full max-w-md rounded-md bg-white p-4 shadow-lg">
        <div class="flex items-start justify-between gap-2">
          <h2 class="text-lg font-semibold text-nt">{{ editingId ? '编辑音乐' : '加一条' }}</h2>
          <a
        ...
        <div class="mt-4 space-y-3">
          <label class="block">
            <span class="text-xs text-nt-soft">曲名</span>
          ...
          <label class="block">
            <span class="text-xs text-nt-soft">艺术家</span>
            <input v-model="form.artist" type="text"
              class="mt-1 w-full rounded border border-nt-divider bg-white px-2 py-1.5 text-sm text-nt outline-none focus:border-nt-accent" />
          </label>
          <label class="block">
            <span class="text-xs text-nt-soft">链接</span>
            <input v-model="form.url" type="url" placeholder="Spotify / YouTube / 网易云…"
              class="mt-1 w-full rounded border border-nt-divider bg-white px-2 py-1.5 text-sm text-nt outline-none focus:border-nt-accent" />
          </label>
          <label class="block">
...
const api = {
  ...
  create: (body)     => fetch('/api/music', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(body) }).then(r => r.json()),
  update: (id, body) => fetch(`/api/music/${id}`, { method:'PATCH', headers:{'Content-Type':'application/json'}, body: JSON.stringify(body) }).then(r => r.json()),
  remove: (id)       => fetch(`/api/music/${id}`, { method:'DELETE' }).then(r => r.json()),
}
...
const editingId = ref('')
const form = ref({ title: '', artist: '', url: '', note: '', rating: 0, cover: null })
...
function openCreate() {
  editingId.value = ''
  form.value = { title: '', artist: '', url: '', note: '', rating: 0, cover: null }
  formOpen.value = true
}
