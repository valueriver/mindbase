...
export const listEmails = (db) =>
  db.prepare(`SELECT ${COLS} FROM emails_addresses ORDER BY created_at DESC`).all()

export const findEmailById = (db, id) =>
  db.prepare(`SELECT ${COLS} FROM emails_addresses WHERE id = ?1`).bind(id).first()

export const insertEmail = async (db, { id, address, label, provider, note }) => {
  await db.prepare(
    `INSERT INTO emails_addresses (id, address, label, provider, note, created_at, updated_at)
...
export const deleteEmail = (db, id) =>
  db.prepare(`DELETE FROM emails_addresses WHERE id = ?1`).bind(id).run()
