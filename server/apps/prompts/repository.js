...
export const listPrompts = (db) =>
  db.prepare(`SELECT ${COLS} FROM prompts_items ORDER BY updated_at DESC`).all()

export const findPromptById = (db, id) =>
  db.prepare(`SELECT ${COLS} FROM prompts_items WHERE id = ?1`).bind(id).first()

export const insertPrompt = async (db, { id, title, content, tags, model, note }) => {
  await db.prepare(
    `INSERT INTO prompts_items (id, title, content, tags, model, note, created_at, updated_at)
...
export const deletePrompt = (db, id) =>
  db.prepare(`DELETE FROM prompts_items WHERE id = ?1`).bind(id).run()
