...
- app_home_posts(id, author, content, created_at, updated_at) — 主页时间轴(用户/AI 共写);author 是身份 slug;content 是文本,可内嵌 markdown 图片 ![](/i/...)
- app_home_events(id, app, action, ref_id, summary, icon, created_at) — 跨应用动作事件流,跟 posts 一起在主页混排
...
## 用户装的应用表(opt-in,从 mindbase.me 应用商店装入)
...
常见命名模式:
- app_<name>_items(收藏 / 银行卡 / 心愿单 等单资源列表)
- app_<name>_entries(记账 / 健康 / 简历 等)
- app_<name>_pages(笔记)、app_<name>_notebooks(笔记的笔记本)
- app_<name>_visits(足迹 / 展览)、app_<name>_shows(演唱会)
- app_<name>_blocks(个人档)
