...
export const updateMovieAction = async (request, env, id) => {
  if (!(await isAuthenticated(request, env))) return fail('unauthorized', 401)
  const existing = await findMovieById(env.DB, id)
  if (!existing) return fail('not_found', 404)
  const body = await readJsonBody(request)
  const row = await updateMovie(env.DB, id, {
    title:  body?.title  !== undefined ? cleanStr(body.title) : undefined,
    year:   body?.year   !== undefined ? cleanInt(body.year) : undefined,
...
export const deleteMovieAction = async (request, env, id) => {
  if (!(await isAuthenticated(request, env))) return fail('unauthorized', 401)
  const existing = await findMovieById(env.DB, id)
  if (!existing) return fail('not_found', 404)
  await deleteMovie(env.DB, id)
  return ok({})
}
