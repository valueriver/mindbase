<template>
  <div>
    <main class="mx-auto w-full max-w-3xl px-4 pt-6 pb-20 md:px-12 md:pt-10">
      <h1 class="text-3xl md:text-[40px] font-bold leading-tight tracking-tight text-nt">上下文</h1>
      <p class="mt-1 text-sm text-nt-soft">所有 AI 协作首先读取的内容。写下你希望 AI 第一时间了解的事。</p>

      <form class="mt-6 flex gap-2" @submit.prevent="onAdd">
        <textarea
          v-model="newContent"
          placeholder="写一条上下文，回车保存"
          rows="2"
      ...
      <div v-else-if="!items.length" class="mt-12 py-8 text-center text-sm text-nt-soft">
        还没有上下文。写一条让 AI 了解你吧 📌
      </div>
        ...
        >
          <div class="flex items-start gap-3">
            <div class="flex-1 min-w-0">
              ...
              />
              <div v-if="item.source_app" class="mt-1 text-xs text-nt-hint">
                来自 {{ item.source_app }}{{ item.source_id ? ` · ${item.source_id}` : '' }}
              </div>
            </div>
            <button
              type="button"
