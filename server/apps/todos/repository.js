...
export const listTodos = (db) =>
  db.prepare(
    `SELECT ${COLS} FROM app_todos_items
      ORDER BY done ASC, sort_order ASC, created_at ASC`
  ).all()

export const findTodoById = (db, id) =>
  db.prepare(`SELECT ${COLS} FROM app_todos_items WHERE id = ?1`).bind(id).first()

export const insertTodo = async (db, { id, title }) => {
  await db.prepare(
    `INSERT INTO app_todos_items (id, title, done, sort_order, created_at, updated_at)
...
export const deleteTodo = (db, id) =>
  db.prepare(`DELETE FROM app_todos_items WHERE id = ?1`).bind(id).run()
