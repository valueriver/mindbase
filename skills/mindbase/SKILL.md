...
- `app_home_events(id, app, action, ref_id, summary, icon, created_at)` —— 跨应用事件流。各应用在关键动作(创建 / 状态跃迁 / 完成里程碑)后,由其后端 service 写入,供主页时间轴渲染。这张表由各应用自身维护,外部 AI 只读。
...
(内置助理与 MCP 客户端用 `sql_query` 工具执行;OpenAPI 客户端用 `POST /api/ai/sql` 端点,语义一致。)
