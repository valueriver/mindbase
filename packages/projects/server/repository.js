...
export const listProjects = (db, { status = null } = {}) => {
  ...
  const sql = `SELECT ${COLS} FROM app_projects_items
...
export const findProjectById = (db, id) =>
  db.prepare(`SELECT ${COLS} FROM app_projects_items WHERE id = ?1`).bind(id).first()

export const insertProject = async (db, { id, title, status, note }) => {
  await db.prepare(
    `INSERT INTO app_projects_items (id, title, status, note, created_at, updated_at)
...
export const deleteProject = (db, id) =>
  db.prepare(`DELETE FROM app_projects_items WHERE id = ?1`).bind(id).run()
