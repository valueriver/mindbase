# ✅ 待办(todos)· MindBase 应用

这是 [MindBase](https://mindbase.me) 应用包,装进 MindBase 实例后,为用户提供"待办"这一面。
...
- **表**(`app_todos_*` 前缀):
  - `app_todos_items`
- **路由**:
  - `/api/todos` 标准 CRUD —— GET 列表、POST 创建、GET/PATCH/DELETE /<id>
  - 子路径:
  - (只有标准 CRUD)
...
装好后,代码位置在 `server/apps/todos/` 与 `gui/apps/todos/`。改完执行 `npm run deploy`。

**通用约定**(与主仓库根 `AGENTS.md` 一致):
- 表名以 `app_todos_*` 为前缀;调整 DDL 时直接修改根 `schema.sql` 并重建数据库,数据迁移由重新建表承担
...
本包是"独立分发的应用源",存放在主仓库的 `packages/todos/` 目录(便于查看与维护),并通过 [mindbase.me/apps/todos](https://mindbase.me/apps/todos) 以 zip 形式分发。安装即将 `packages/` 中的对应文件复制到 `server/apps/` 与 `gui/apps/` 的相应位置。
