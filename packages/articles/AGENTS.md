...
装好后,代码位置在 `server/apps/articles/` 与 `gui/apps/articles/`。改完执行 `npm run deploy`。
...
本包是"独立分发的应用源",存放在主仓库的 `packages/articles/` 目录(便于查看与维护),并通过 [mindbase.me/apps/articles](https://mindbase.me/apps/articles) 以 zip 形式分发。安装即将 `packages/` 中的对应文件复制到 `server/apps/` 与 `gui/apps/` 的相应位置。
