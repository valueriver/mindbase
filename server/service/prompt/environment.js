...
- ledger(id, type, amount, category, note, happened_at, created_at, updated_at) — 记账流水
  type ∈ {'expense','income'};amount 是整数"分"(100 = 1 元);happened_at 是 'YYYY-MM-DD'
  常见查询:SELECT SUM(amount)/100.0 AS yuan FROM ledger WHERE type='expense' AND happened_at LIKE '2026-05%'
