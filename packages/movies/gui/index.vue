  ...
  <div>
    <main class="mx-auto w-full max-w-3xl px-4 pt-6 pb-20 md:px-12 md:pt-10">
      <div class="flex items-end justify-between gap-4">
        <div>
          <h1 class="text-3xl md:text-[40px] font-bold leading-tight tracking-tight text-nt">电影</h1>
          <p class="mt-1 text-sm text-nt-soft">想看的、在看的、看过的。</p>
        </div>
        <button
          type="button"
      ...
      <ul v-else class="mt-6 divide-y divide-nt-divider rounded-md border border-nt-divider bg-white">
        <li
          ...
          @click="openEdit(it)"
        >
          <img v-if="it.cover" :src="it.cover" class="h-9 w-6 shrink-0 rounded object-cover" alt="" />
          <div class="min-w-0 flex-1">
            <div class="flex items-center gap-2">
              <span class="truncate text-sm font-medium text-nt">
                {{ it.title }}<span v-if="it.year" class="text-nt-soft font-normal"> ({{ it.year }})</span>
              </span>
    ...
    <div v-if="formOpen" class="fixed inset-0 z-40 flex items-center justify-center bg-black/30 px-4" @click.self="closeForm">
      <div class="w-full max-w-md rounded-md bg-white p-4 shadow-lg">
        <h2 class="text-lg font-semibold text-nt">{{ editingId ? '编辑电影' : '加一条' }}</h2>
        <div class="mt-4 space-y-3">
          <label class="block">
            <span class="text-xs text-nt-soft">片名</span>
          ...
          <div class="flex gap-2">
            <label class="block w-28">
              <span class="text-xs text-nt-soft">年份</span>
...
const api = {
  list:   (qs = '')  => fetch(`/api/movies${qs}`).then(r => r.json()),
  create: (body)     => fetch('/api/movies', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(body) }).then(r => r.json()),
  update: (id, body) => fetch(`/api/movies/${id}`, { method:'PATCH', headers:{'Content-Type':'application/json'}, body: JSON.stringify(body) }).then(r => r.json()),
  remove: (id)       => fetch(`/api/movies/${id}`, { method:'DELETE' }).then(r => r.json()),
}
...
const editingId = ref('')
const form = ref({ title: '', year: null, status: 'want', note: '', rating: 0, cover: null })

const canSave = computed(() => form.value.title.trim().length > 0)

function statusLabel(s) { return { want: '想看', watching: '在看', watched: '看过' }[s] || s }
...
function openCreate() {
  editingId.value = ''
  form.value = { title: '', year: null, status: 'want', note: '', rating: 0, cover: null }
  formOpen.value = true
}
...
async function onSave() {
  ...
  const body = {
    ...
    year:   Number.isFinite(form.value.year) ? form.value.year : null,
