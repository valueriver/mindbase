  ...
  <div>
    <main class="mx-auto w-full max-w-3xl px-4 pt-6 pb-20 md:px-12 md:pt-10">
      <div class="flex items-end justify-between gap-4">
        <div>
          <h1 class="text-3xl md:text-[40px] font-bold leading-tight tracking-tight text-nt">菜谱</h1>
          <p class="mt-1 text-sm text-nt-soft">想做的菜、做过的菜。</p>
        </div>
        <button type="button"
          class="shrink-0 rounded-md bg-nt px-3 py-1.5 text-sm text-white hover:bg-nt-strong"
          @click="openCreate">＋ 加一道</button>
      </div>
      ...
      <div v-else-if="!items.length" class="mt-10 text-center text-sm text-nt-soft">还没有菜谱。</div>
        ...
        <li v-for="it in items" :key="it.id"
          ...
          <div class="min-w-0 flex-1">
            <div class="flex flex-wrap items-center gap-2">
              ...
              <span v-if="it.cuisine" class="rounded bg-nt-hover px-1.5 py-0.5 text-[10px] text-nt-soft">{{ it.cuisine }}</span>
            </div>
            <div class="mt-0.5 flex gap-3 text-xs text-nt-soft">
              <span v-if="it.servings > 0">{{ it.servings }} 人份</span>
              <span v-if="it.prep_min > 0">{{ it.prep_min }} 分钟</span>
            </div>
          </div>
        </li>
      </ul>
    ...
    <div v-if="formOpen" class="fixed inset-0 z-40 flex items-center justify-center bg-black/30 px-4" @click.self="closeForm">
      <div class="w-full max-w-md rounded-md bg-white p-4 shadow-lg max-h-[90vh] overflow-y-auto">
        <h2 class="text-lg font-semibold text-nt">{{ editingId ? '编辑菜谱' : '加一道' }}</h2>
        <div class="mt-4 space-y-3">
          <label class="block">
            <span class="text-xs text-nt-soft">菜名</span>
          ...
          <div class="flex gap-2">
            <label class="block flex-1">
              <span class="text-xs text-nt-soft">菜系</span>
              <input v-model="form.cuisine" type="text" placeholder="川菜 / 家常"
                class="mt-1 w-full rounded border border-nt-divider bg-white px-2 py-1.5 text-sm text-nt outline-none focus:border-nt-accent" />
            </label>
            <label class="block w-24">
              <span class="text-xs text-nt-soft">人份</span>
              <input v-model.number="form.servings" type="number" min="0"
                class="mt-1 w-full rounded border border-nt-divider bg-white px-2 py-1.5 text-sm text-nt outline-none focus:border-nt-accent" />
            </label>
            <label class="block w-28">
              <span class="text-xs text-nt-soft">耗时(分)</span>
              <input v-model.number="form.prep_min" type="number" min="0"
                class="mt-1 w-full rounded border border-nt-divider bg-white px-2 py-1.5 text-sm text-nt outline-none focus:border-nt-accent" />
            </label>
          </div>
          <label class="block">
            <span class="text-xs text-nt-soft">食材</span>
            <textarea v-model="form.ingredients" rows="6"
              class="mt-1 w-full rounded border border-nt-divider bg-white px-2 py-1.5 font-mono text-xs text-nt outline-none focus:border-nt-accent"></textarea>
          </label>
          <label class="block">
            <span class="text-xs text-nt-soft">步骤</span>
            <textarea v-model="form.steps" rows="6"
              class="mt-1 w-full rounded border border-nt-divider bg-white px-2 py-1.5 font-mono text-xs text-nt outline-none focus:border-nt-accent"></textarea>
          </label>
          <label class="block">
...
const api = {
  ...
  create: (body)     => fetch('/api/recipes', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(body) }).then(r => r.json()),
  update: (id, body) => fetch(`/api/recipes/${id}`, { method:'PATCH', headers:{'Content-Type':'application/json'}, body: JSON.stringify(body) }).then(r => r.json()),
  remove: (id)       => fetch(`/api/recipes/${id}`, { method:'DELETE' }).then(r => r.json()),
}
...
const editingId = ref('')
const blank = () => ({ title: '', cuisine: '', servings: 0, prep_min: 0, ingredients: '', steps: '', note: '', cover: null })
...
function openEdit(it) {
  ...
  form.value = {
    title: it.title || '', cuisine: it.cuisine || '',
    servings: it.servings ?? 0, prep_min: it.prep_min ?? 0,
    ingredients: it.ingredients || '', steps: it.steps || '',
...
async function onSave() {
  ...
  const body = {
    ...
    cuisine: form.value.cuisine.trim(),
    servings: Number(form.value.servings) || 0,
    prep_min: Number(form.value.prep_min) || 0,
...
async function onDelete() {
  if (!window.confirm('删除这道菜?')) return
  busy.value = true
  try {
