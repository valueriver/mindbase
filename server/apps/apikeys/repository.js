...
export const updateApikey = async (db, id, patch) => {
  ...
  if (sets.length) {
    ...
    await db.prepare(`UPDATE apikeys_items SET ${sets.join(', ')} WHERE id = ?1`).bind(...binds).run()
  }
  return findApikeyById(db, id)
}

export const deleteApikey = (db, id) =>
  db.prepare(`DELETE FROM apikeys_items WHERE id = ?1`).bind(id).run()
