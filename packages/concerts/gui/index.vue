  ...
  <div>
    <main class="mx-auto w-full max-w-3xl px-4 pt-6 pb-20 md:px-12 md:pt-10">
      <div class="flex items-end justify-between gap-4">
        <div>
          <h1 class="text-3xl md:text-[40px] font-bold leading-tight tracking-tight text-nt">演唱会</h1>
          <p class="mt-1 text-sm text-nt-soft">看过的现场。</p>
        </div>
        <button type="button"
          class="shrink-0 rounded-md bg-nt px-3 py-1.5 text-sm text-white hover:bg-nt-strong"
          @click="openCreate">＋ 加一场</button>
      </div>
        ...
        <li v-for="it in items" :key="it.id"
          ...
          <div class="min-w-0 flex-1">
            <div class="flex flex-wrap items-center gap-2">
              <span class="truncate text-sm font-medium text-nt">{{ it.artist }}</span>
              <span v-if="it.tour" class="truncate text-xs text-nt-soft">{{ it.tour }}</span>
            ...
            <div class="mt-0.5 flex flex-wrap items-center gap-2 text-xs text-nt-soft">
              <span v-if="it.venue || it.city">{{ [it.venue, it.city].filter(Boolean).join(' · ') }}</span>
              <span v-if="it.show_date">{{ it.show_date }}</span>
              <span v-if="it.seat">座位 {{ it.seat }}</span>
              <span v-if="it.ticket_price > 0">¥{{ (it.ticket_price / 100).toFixed(2) }}</span>
            </div>
          </div>
        </li>
      </ul>
    ...
    <div v-if="formOpen" class="fixed inset-0 z-40 flex items-center justify-center bg-black/30 px-4" @click.self="closeForm">
      <div class="w-full max-w-md rounded-md bg-white p-4 shadow-lg max-h-[90vh] overflow-y-auto">
        <h2 class="text-lg font-semibold text-nt">{{ editingId ? '编辑' : '加一场' }}</h2>
        <div class="mt-4 space-y-3">
          <label class="block">
            <span class="text-xs text-nt-soft">歌手 / 乐队</span>
          ...
          <label class="block">
            <span class="text-xs text-nt-soft">巡演名</span>
            <input v-model="form.tour" type="text"
              class="mt-1 w-full rounded border border-nt-divider bg-white px-2 py-1.5 text-sm text-nt outline-none focus:border-nt-accent" />
          </label>
          <div class="flex gap-2">
            <label class="block flex-1">
              <span class="text-xs text-nt-soft">场馆</span>
          ...
          <div class="flex gap-2">
            <label class="block flex-1">
              ...
              <input v-model="form.show_date" type="date"
                class="mt-1 w-full rounded border border-nt-divider bg-white px-2 py-1.5 text-sm text-nt outline-none focus:border-nt-accent" />
            </label>
            <label class="block flex-1">
              <span class="text-xs text-nt-soft">座位</span>
              <input v-model="form.seat" type="text"
                class="mt-1 w-full rounded border border-nt-divider bg-white px-2 py-1.5 text-sm text-nt outline-none focus:border-nt-accent" />
            </label>
            <label class="block flex-1">
              <span class="text-xs text-nt-soft">票价 (元)</span>
          ...
          <div class="flex items-center gap-2 text-sm text-nt-soft">
            ...
            <button v-for="n in 5" :key="n" type="button"
              ...
              @click="form.rating = (form.rating === n ? 0 : n)">★</button>
          </div>
          <label class="block">
...
const api = {
  list:   ()         => fetch('/api/concerts').then(r => r.json()),
  create: (body)     => fetch('/api/concerts', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(body) }).then(r => r.json()),
  update: (id, body) => fetch(`/api/concerts/${id}`, { method:'PATCH', headers:{'Content-Type':'application/json'}, body: JSON.stringify(body) }).then(r => r.json()),
  remove: (id)       => fetch(`/api/concerts/${id}`, { method:'DELETE' }).then(r => r.json()),
}
...
const editingId = ref('')
const blank = () => ({ artist: '', tour: '', venue: '', city: '', show_date: '', priceYuan: 0, seat: '', rating: 0, note: '', cover: null })
const form = ref(blank())

const canSave = computed(() => form.value.artist.trim().length > 0)
...
function openEdit(it) {
  ...
  form.value = {
    artist: it.artist || '', tour: it.tour || '',
    venue: it.venue || '', city: it.city || '',
    show_date: it.show_date || '',
    priceYuan: (it.ticket_price ?? 0) / 100,
    seat: it.seat || '', rating: it.rating || 0,
...
async function onSave() {
  ...
  const body = {
    ...
    tour: form.value.tour.trim(),
    venue: form.value.venue.trim(),
    city: form.value.city.trim(),
    show_date: form.value.show_date || null,
    ticket_price: Math.round((Number(form.value.priceYuan) || 0) * 100),
    seat: form.value.seat.trim(),
