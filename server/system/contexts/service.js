...
export const createContextAction = async (request, env) => {
  ...
  const row = await insertContext(env.DB, { id: randomId(), content, sourceApp: '', sourceId: null, sortOrder: maxOrder + 10 })
...
export const pinContextAction = async (request, env) => {
  ...
  if (!content || !sourceApp || !sourceId) return fail('content_source_app_source_id_required', 400)
  const all      = await listContexts(env.DB)
  const maxOrder = (all?.results || []).reduce((m, r) => Math.max(m, r.sort_order ?? 0), 0)
  const row = await upsertContext(env.DB, { id: randomId(), content, sourceApp, sourceId, sortOrder: maxOrder + 10 })
  return ok({ context: serialize(row) })
}
